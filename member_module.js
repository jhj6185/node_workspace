// const { authMember, addMember, listMember } = require("./member_sj");

var printListMember = function(results,res){
...
var addMember= function(database, userId, userPwd,userName,age, callback){
    ...
    //MemberModel 인슨턴스 생성
    var user= new database.MemberModel({"userId": userId, "userPwd": userPwd, 
    "userName": userName, 
    "age": age});
...
var updateMember = function(database, userId, userPwd,userName,age, callback){
    ...
    database.MemberModel.updateMany({ "userId": userId },{ $set: {"userPwd": userPwd,
        "userName": userName,"age":age }}, function(err, result){
        if(err){
